...
var require_before_after_hook = __commonJS({
  "node_modules/before-after-hook/index.js"(exports, module) {
    ...
    function HookCollection() {
      ...
      var hook2 = register.bind(null, state2);
...
function withDefaults2(oldEndpoint, newDefaults) {
  ...
  const ret = Object.assign(newApi, {
    endpoint: endpoint2,
    defaults: withDefaults2.bind(null, endpoint2)
  });
  return ret;
...
function toApiArgs(args, initialValue = {}) {
  return Object.keys(args).filter((key) => args[key] !== void 0).reduce((acc, key) => {
    if (key === "pageSize") {
...
async function timer({ key }) {
  const { state: state2 } = program;
  const [owner, repo] = key.split("/");
  const { data, meta } = await client.activity.getEventsForRepo({ owner, repo });
  const index = data.findIndex((item) => formatTime(item.created_at) <= state2.repos[key].lastEventTime);
  if (index > 0) {
    const newEvents = data.slice(0, index).reverse();
    for (let item of newEvents) {
      const { type, payload } = item;
      for (let eventData of state2.repos[key].events) {
    ...
    state2.repos[key].lastEventTime = formatTime(lastEvent.created_at);
    await program.save();
  }
...
async function ensureTimerIsSet(repo, event) {
  const { state: state2 } = program;
  const repository = state2.repos[repo] = state2.repos[repo] || {};
